...
export default function Admin() {
  ...
  const deleteUser = async (id) => {
    try { await api.delete(`/admin/users/${id}`, { headers: { Authorization: `Bearer ${token}` } }); setUsers(u => u.filter(x => x._id !== id)); }
    catch (e) { setError(e?.response?.data?.error || e.message); }
  };

  const verify = async (id, st) => {
    try {
      const r = await api.put(`/admin/charities/${id}/verify`, { status: st }, { headers: { Authorization: `Bearer ${token}` } });
    ...
    <div className="page-shell">
        ...
        <div>
          <div className="filter-bar">
            <div style={{ flex: 2, minWidth: 240 }}>
              ...
              <input className="gh-input" placeholder="Name, description…" value={q} onChange={e => setQ(e.target.value)} />
            </div>
          ...
          <div className="gh-card" style={{ overflow: 'hidden' }}>
            <div style={{ overflowX: 'auto' }}>
              <table className="gh-table">
                <thead><tr><th>Name</th><th>Category</th><th>Submitted By</th><th>Status</th><th>Actions</th></tr></thead>
                <tbody>
                  {charities.map(c => (
                    <tr key={c._id}>
      ...
      {activeTab === 'users' && (
        <div className="gh-card" style={{ overflow: 'hidden' }}>
          <div style={{ overflowX: 'auto' }}>
            <table className="gh-table">
              <thead><tr><th>Name</th><th>Email</th><th>Admin</th><th>Joined</th><th>Action</th></tr></thead>
              <tbody>
                {users.map(u => (
                  <tr key={u._id}>
                    ...
                    <td>{!u.isAdmin && <button onClick={() => deleteUser(u._id)} style={btnStyle('#FEE2E2', '#991B1B')}>Delete</button>}</td>
                  </tr>
                ))}
              </tbody>
            </table>
...
const btnStyle = (bg, color) => ({ background: bg, color, border: 'none', borderRadius: 6, padding: '0.35rem 0.7rem', fontSize: '0.78rem', fontWeight: 700, cursor: 'pointer', fontFamily: 'var(--font-body)' });
const pgBtn = { background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 8, padding: '0.5rem 1rem', cursor: 'pointer', fontSize: '0.85rem', color: 'var(--ink-mid)', fontFamily: 'var(--font-body)', fontWeight: 500 };
